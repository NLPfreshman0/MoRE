...
class LoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora_A", "lora_B", "lora_embedding_A", "lora_embedding_B", "lora_gate", "lora_task_embedding")
...
class Linear(nn.Module, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_experts = 8
        self.lora_gate = nn.ParameterDict({})
        self.lora_task_embedding = nn.ParameterDict({})
    ...
    def init_task_map(self):
        for i in range(8):
            task_embedding = self.lora_task_embedding['default'](torch.LongTensor([i]).to(self.lora_task_embedding['default'].weight.device))
            expert_logits = self.lora_gate['default'](task_embedding).squeeze(0)
            approximated_logits = self.straight_through_argmax(expert_logits)
            expert_ids = torch.argmax(approximated_logits)
            self.task_map[i] = expert_ids
    ...
    def forward(self, x: torch.Tensor, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                scaling = self.scaling[active_adapter]
                if not self.init:
                    task_embedding = self.lora_task_embedding[active_adapter](torch.LongTensor([get_task_id()]).to(x.device))
                    
                    expert_logits = self.lora_gate[active_adapter](task_embedding).squeeze(0)
                    approximated_logits = self.straight_through_argmax(expert_logits)
                    expert_ids = torch.argmax(approximated_logits)
                ...
                # 计算所有样本之间的相似度
                #similarity_matrix = torch.matmul(x[:, 0, :], self.lora_task_embedding[active_adapter].weight.t()) / temperature
                similarity_matrix = torch.matmul(x_norm, task_norm.t()) / temperature
                
                # task_ids = torch.argmax(similarity_matrix, dim=1)
                # #print(task_ids)
                # task_embedding = self.lora_task_embedding[active_adapter](torch.LongTensor([task_ids]).to(x.device))
