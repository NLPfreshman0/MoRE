...
class T5Stack(T5PreTrainedModel):
    ...
    def forward(
        ...
        #cl_loss
        #print(layer_module)
        cl_loss = torch.tensor(0.0, dtype=layer_module.layer[0].SelfAttention.q.cl_loss.dtype, device=layer_module.layer[0].SelfAttention.q.cl_loss.device)
        for i, layer_module in enumerate(self.block):
            cl_loss += layer_module.layer[0].SelfAttention.q.cl_loss
            cl_loss += layer_module.layer[0].SelfAttention.k.cl_loss
            cl_loss += layer_module.layer[0].SelfAttention.v.cl_loss
            cl_loss += layer_module.layer[0].SelfAttention.o.cl_loss
            if self.is_decoder:
                cl_loss += layer_module.layer[1].EncDecAttention.q.cl_loss
                cl_loss += layer_module.layer[1].EncDecAttention.k.cl_loss
                cl_loss += layer_module.layer[1].EncDecAttention.v.cl_loss
                cl_loss += layer_module.layer[1].EncDecAttention.o.cl_loss
                cl_loss += layer_module.layer[2].DenseReluDense.wi.cl_loss
                cl_loss += layer_module.layer[2].DenseReluDense.wo.cl_loss
            else:
                cl_loss += layer_module.layer[1].DenseReluDense.wi.cl_loss
                cl_loss += layer_module.layer[1].DenseReluDense.wo.cl_loss
        if self.is_decoder:
            cl_loss /= (12 * 10)
        else:
            cl_loss /= (12 * 6)
